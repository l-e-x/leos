...
module.exports = class BucketBar extends Plugin
  ...
  _update: ->
    ...
    sidebarContainerRect = @annotator.frame[0].getBoundingClientRect() #LEOS Change taking account of sidebar frame position
    ...
    points = @annotator.anchors.reduce (points, anchor, i) =>
      ...
      rect = highlighter.getBoundingClientRect(anchor.highlights)
      x = rect.top - sidebarContainerRect.top                            #LEOS Change - alignement of the pins: takes account of sidebar position and height compared to the window
    ...
    # Scroll down
    @buckets.push [], below, []
    @index.push sidebarContainerRect.height - BUCKET_NAV_SIZE,    #LEOS Change alignement of the bottom pin: takes account of sidebar height - NOT window height
    ...
    $.each @buckets.slice(@tabs.length), =>
      ...
      .on 'click', (event) =>
        ...
        # If it's the lower tab, scroll to next anchor below
          ...
          @annotator.selectAnnotations annotations,  #LEOS Change: correction for null pointer exception
