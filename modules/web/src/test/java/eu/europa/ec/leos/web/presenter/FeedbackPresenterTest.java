...
public class FeedbackPresenterTest extends LeosPresenterTest {
    ...
    public void testEnterFeedbackView() throws Exception {
        ...
        verify(feedbackView).setData(commentService.getAllComments(document));
    ...
    public void testRefreshDocument() throws IOException, ParseException {
        ...
        verify(feedbackView).refreshContent(displayableContent);
        verify(feedbackView).setDocumentTitle(docName);
        verify(feedbackView).setData(comments);
        verify(feedbackView).setDocumentStage(document.getStage());
        verify(feedbackView).setToc(argThat(sameInstance(tableOfContentItemVoList)));
    ...
    @Test
    public void test_insertComment_Before() throws Exception {
        //setup
        String docId = "555";
        ...
        when(commentService.insertNewComment(originalDocument, elementId, commentId, commentContent, true)).thenReturn(updatedDocument);

        // DO THE ACTUAL CALL
        feedbackPresenter.insertComments(new InsertCommentEvent(elementId,commentId,commentContent));

        verify(commentService).insertNewComment(originalDocument, elementId, commentId, commentContent, true);
        verify(documentService).getDocument(docId);
