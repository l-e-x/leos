 ...
 */
; // jshint ignore:line
define(function aknLevelOrderedListPluginModule(require) {
    "use strict";
    ...
    var pluginName = "aknLevelOrderedList";
    ...
    var ORDERED_LIST_SELECTOR = "ol[data-akn-name='aknLevelOrderedList']";
    var ENTER_KEY = 13;
    var TAB_KEY = 9;
    ...
    function _onEnterKey(context) {
        ...
        if ((leosKeyHandler.isContentEmptyTextNode(selectedElement) && leosPluginUtils.isSelectionInFirstLevelList(selectedElement))
            || leosPluginUtils.isLevelList(selectedElement.getAscendant('ol', true))) {
    ...
    function _restoreListStructure(event) {
        if (event.data.name === 'enter') {
            ...
            var isLevelElement = leosPluginUtils.isLevelList(parent);
    ...
    function _resetDataNumOnIndent(event) {
        ...
        while ((range = iterator.getNextRange())) {
            if (range.startContainer) {
                ...
                if (range.startContainer.type === CKEDITOR.NODE_TEXT && startNode.getParent().getName() === 'ol' && startNode.getParent().getAttribute('data-akn-name') === 'aknLevel') {
            ...
            if (range.endContainer) {
                ...
                if (range.endContainer.type === CKEDITOR.NODE_TEXT && endNode.getParent().getName() === 'ol' && endNode.getParent().getAttribute('data-akn-name') === 'aknLevel') {
    ...
    function _handleNode(node) {
        if (!node || node.type !== CKEDITOR.NODE_ELEMENT || node.getParent().getAttribute('data-akn-name') === 'aknLevel'){
            return;
        }
    ...
    function resetDataAknNameForOrderedList(event) {
        ...
        for (var ii = 0; ii < orderedLists.length; ii++) {
            ...
            if (currentNestingLevel > 0) {
                orderedList.setAttribute("data-akn-name", "aknLevelOrderedList");
    ...
    var leosHierarchicalElementTransformer = leosHierarchicalElementTransformerStamp({
        firstLevelConfig : {
            akn : 'list',
            html : 'ol[data-akn-name=aknLevelOrderedList]',
            attr : [ {
                akn : "leos:editable",
                ...
                html : "data-akn-name=aknLevelOrderedList"
            } ]
        },
