...
; // jshint ignore:line
define(function aknLevelListPluginModule(require) {
    "use strict";
    ...
    CKEDITOR.plugins.leosLevelList = {
        /**
        ...
        listToArray: function (listNode, database, baseArray, baseIndentLevel, grandparentNode) {
            ...
            for (var i = 0, count = listNode.getChildCount(); i < count; i++) {
                ...
                if (_isLevelList(listNode)) {
                    var listOfElement = listNode.getChildren().toArray().find(element => element.getName() === 'li');
                    for (var i = 0, count = listOfElement.getChildCount(); i < count; i++) {
                        var listItem = listOfElement.getChild(i);
                        var itemObj = {
                            'parent': listOfElement,
                            indent: baseIndentLevel,
                            element: listItem,
                            contents: [],
                            indentThisElement: false
                        };
                        itemObj.grandparent = listNode;
                        if (database) {
                            CKEDITOR.dom.element.setMarker(database, listItem, 'listarray_index', baseArray.length);
                        }
                        baseArray.push(itemObj);
                        
                        // in case of node elements iterate the children, otherwise add the founded text node to the result
                        if(listItem.type === CKEDITOR.NODE_ELEMENT) {
                            for (var j = 0, itemChildCount = listItem.getChildCount(), child; j < itemChildCount; j++) {
                                child = listItem.getChild(j);
                                itemObj.contents.push(child);
                            }
                        } else if(listItem.type === CKEDITOR.NODE_TEXT){
                            itemObj.contents.push(listItem);
                        } else {
                            LOG.warn("Unhandled type of node type: " + listItem.type);
                        }
                    }
                    break;
                } else {
                    ...
                    if (listItem.type == CKEDITOR.NODE_ELEMENT && listItem.getName() in CKEDITOR.dtd.$list)
                        CKEDITOR.plugins.leosLevelList.listToArray(listItem, database, baseArray, baseIndentLevel + 1);
                    ...
                    for (var j = 0, itemChildCount = listItem.getChildCount(), child; j < itemChildCount; j++) {
                        child = listItem.getChild(j);
                        if (child.type == CKEDITOR.NODE_ELEMENT && listNodeNames[child.getName()] && !(_isLevelList(itemObj.grandparent)))// Note the recursion here, it pushes inner list items with// +1 indentation in the correct order.
                            CKEDITOR.plugins.leosLevelList.listToArray(child, database, baseArray, baseIndentLevel + 1, itemObj.grandparent);
                        else
                            itemObj.contents.push(child);
                    }
                }
        ...
        arrayToList: function( listArray, database, baseIndex, paragraphMode, dir ) {
            ...
            while ( 1 ) {
                ...
                if (item.indent == indentLevel) {
                    var isRootAppended = false;
                    if ((!rootNode || listArray[currentIndex].parent.getName() != rootNode.getName())) {
                        if (listArray[currentIndex].element.getName() === 'ol' && listArray[currentIndex].grandparent.getAttribute('data-akn-name') === 'aknLevel') {
                            rootNode = listArray[currentIndex].element.clone(false, 1);
                            isRootAppended = true;
                        } else {
     ...
     // Maintain original direction (http://dev.ckeditor.com/ticket/6861).
                    var currDir = listArray[ currentIndex - 1 ].element.getDirection( 1 ),
                        listData = CKEDITOR.plugins.leosLevelList.arrayToList( listArray, null, currentIndex, paragraphMode, currDir != orgDir ? orgDir : null );
     ...
     // wouldn't be editable. (http://dev.ckeditor.com/ticket/6724)
                    ...
                    if (listArray[currentIndex].indentThisElement) {
                        if (_isLevelOrderedList(currentListItem) && listData.listNode.getChildCount() == 1) {
                                var olElement = listData.listNode.getChildren().toArray()[0];
                                for (var i = 0; i < olElement.getChildCount(); i++)
                                    currentListItem.append(olElement.getChild(i).clone(1,1));
                        } else {
                            if(listArray[currentIndex].element.getAttribute('data-akn-mp-tag-name') === 'p'){
                                listData.listNode.insertAfterNode(currentListItem);
                            }else{
                                currentListItem.append(listData.listNode);
                            }
                        }
                    } else {
                        if (_isLevelOrderedList(currentListItem.getParent()) && listData.listNode.getChildCount() == 1 && currentListItem.findOne('ol')) {
                            var olElement = listData.listNode.getChildren().toArray()[0];
                            for (var i = 0; i < olElement.getChildCount(); i++)
                                currentListItem.findOne('ol').append(olElement.getChild(i).clone(1, 1));
                        } else {
                            currentListItem.append(listData.listNode);
                        }
                    }
                    ...
                    var aknLevelElement = _isLevelList(item.grandparent);/// && !_isLevelOrderedList(item.parent);
                    ...
                    for ( i = 0; i < count; i++ ) {
                        ...
                        // Block content goes directly to the current list item, without wrapping.
                        else if ((child.type == CKEDITOR.NODE_ELEMENT && child.isBlockBoundary()) || (_isLevelList(item.grandparent))) {
                            ...
                            if(!aknLevelElement && child.type == CKEDITOR.NODE_ELEMENT && !child.is('br')){
                                currentListItem.append( child.clone( 1, 1 ) );
                            }else{
                                var docFragment = new CKEDITOR.dom.documentFragment(doc);
                                var olElement;
                                var newElement = new CKEDITOR.dom.element('p');
                                var isNewElementCreated = false;
                                var isListElementPresent = false

                                var childSiblings = child.getParent().getChildren().toArray();

                                for (var i = 0; i < childSiblings.length; i++) {
                                    if (childSiblings[i].type === CKEDITOR.NODE_ELEMENT && _isLevelOrderedList(childSiblings[i])) {
                                        olElement = childSiblings[i].clone(1, 1);
                                        isListElementPresent = true;
                                    } else if (childSiblings[i].type === CKEDITOR.NODE_ELEMENT && childSiblings[i].getName() === 'p') {
                                        docFragment.append(childSiblings[i].clone(1, 1));
                                    } else {
                                        newElement.append(childSiblings[i].clone(1, 1));
                                        isNewElementCreated = true;
                                    }
                                }
                                if (isNewElementCreated) {
                                    newElement.setAttributes(child.getParent().getAttributes());
                                    if(!child.getParent().getAttribute('data-akn-mp-tag-name')){
                                        newElement.setAttribute('data-akn-mp-tag-name','p');
                                    }
                                    docFragment.append(newElement);
                                }
                                if (isListElementPresent) {
                                    docFragment.append(olElement);
                                }
                                currentListItem = docFragment;
                                break;
                            }
                        }
                    ...
                    if ( (currentListItemName == 'div' || currentListItemName == 'p') && !aknLevelElement) {
    ...
    function _isLevelOrderedList(element){ return element && 'aknLevelOrderedList' === element.getAttribute('data-akn-name'); }

    function _isLevelList(element) { return element && 'aknLevel' === element.getAttribute('data-akn-name'); }
    ...
    function changeListType( editor, groupObj, database, listsCreated ) {
        ...
        var listArray = CKEDITOR.plugins.leosLevelList.listToArray( groupObj.root, database ),
            selectedListItems = [];
        ...
        var newList = CKEDITOR.plugins.leosLevelList.arrayToList( listArray, database, null, editor.config.enterMode );
        var child,
    ...
    function removeList( editor, groupObj, database ) {
        ...
        var newList = CKEDITOR.plugins.leosLevelList.arrayToList( listArray, database, null, editor.config.enterMode, groupObj.root.getAttribute( 'dir' ) );
