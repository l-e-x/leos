...
public class TemplateStructureServiceImpl implements TemplateStructureService {
    ...
    public byte[] getStructure(String templateID) {
        JsonNode structureXML = templateConfigurationService.getTemplateConfigurationJson(templateID, structure);
        XmlDocument structureXmlDocument = configurationRepository.findTemplate(templatesStructurePath, structureXML.get(0).get("name").asText());
